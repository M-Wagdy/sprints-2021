...
/*- LOCAL MACROS
------------------------------------------*/
#define  CAR_FORWARD_SPEED              (uint16_t)( 15 )
#define  CAR_BACKWARD_SPEED             (uint16_t)( 15 )
#define  CAR_ROTATERIGHT_SPEED          (uint16_t)( 100 )
...
extern ERROR_STATE_t CRASH_Logic(double64_t gd64_distance, uint8_t * CarDirection, uint16_t * CarSpeed)
{
   ...
   if(u8_PreviousState != u8_CurrentState)
   {
      ...
      {
         case CRASH_Forward:
            *CarDirection = STEERING_FORWARD;
            *CarSpeed = CAR_FORWARD_SPEED;
            u8_PreviousState = u8_CurrentState;
            break;
         case CRASH_Backward:
            *CarDirection = STEERING_BACKWARD;
            *CarSpeed = CAR_BACKWARD_SPEED;
            u8_PreviousState = u8_CurrentState;
            break;
         case CRASH_RotateRight:
            *CarDirection = STEERING_RIGHT;
            *CarSpeed = CAR_ROTATERIGHT_SPEED;
            u8_PreviousState = u8_CurrentState;
            break;
         default:
            return ERROR_NOK;
...
extern void CRASH_MainFunction(void)
{
   ...
   CRASH_MainErrorState = CRASH_Logic(gd64_distance, &gu8_CarDirection, &gu8_CarSpeed);
