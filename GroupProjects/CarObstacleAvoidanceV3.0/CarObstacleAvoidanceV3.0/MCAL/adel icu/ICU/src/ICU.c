...
static Ptr_VoidFuncVoid_t gptr_CallbackInputCapture=NULL_PTR;
static Ptr_VoidFuncVoid_t gptr_CallbackOverflow=NULL_PTR;
...
ERROR_STATE_t ICU_UpdateTimeOn(uint8_t ICUNumber,ptr_double64_t TimeOnValue)
{
	uint8_t u8_ErrorState=ERROR_NOK;
	static uint8_t ICUState = START_STATE;
	if((NULL_PTR==TimeOnValue))
	{
		u8_ErrorState = ERROR_NOK;
	}
					...
					switch(ICUState)
					{
						...
						case WAITING_FALLING_EDGE_STATE:
						{
							...
							if(ICU_FLAG_UP==(BIT_GET(TIFR,ICF1)))
							{
								...
								{
                           SET_BIT(TIFR,ICF1);
									ICUState=START_STATE; /*edited by me*/
									u8_ErrorState = ERROR_OK;	
								}
