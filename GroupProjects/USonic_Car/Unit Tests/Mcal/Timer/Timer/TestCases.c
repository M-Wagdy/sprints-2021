...
static TIMER_ERROR_state_t Timer_ErrorState;
...
void TestInit(void)
{
	...
	if (
		(Timer_ErrorState == TIMER_E_SUCCESS) && (TIMER_0_CONTROL == gastr_Timer_Config[TIMER_0].u8_mode) &&
		(TIMER_INT_MASK == gastr_Timer_Config[TIMER_0].u8_interrupt_mode)
		)
	{
		printf("Test Case ID: %u - PASSED \n", u8_ID);
		u8_PassedCounter++;
...
static void TestStart(void)
{
	...
	if (
		(Timer_ErrorState == TIMER_E_SUCCESS) && (TIMER_0_DATA == 20)
		)
	{
		printf("Test Case ID: %u - PASSED \n", u8_ID);
		u8_PassedCounter++;
...
void TestStop(void)
{
	...
	if (
		(Timer_ErrorState == TIMER_E_SUCCESS) && !(TIMER_0_CONTROL & CLK_BITS)
		)
	{
		printf("Test Case ID: %u - PASSED \n", u8_ID);
		u8_PassedCounter++;
...
void TestGetStatus(void)
{
	...
	Timer_ErrorState = TIM_GetStatus(TIMER_0, &u8_State);
	if (
		(Timer_ErrorState == TIMER_E_SUCCESS) && (u8_State == TIMER_OVERFLOW_FLAG)
		)
	{
		printf("Test Case ID: %u - PASSED \n", u8_ID);
		u8_PassedCounter++;
...
void TestDelayUs(void)
{
	...
	TIMER_0_DATA = 0x00;
	/* Clear Clock Bits */
	TIMER_0_CONTROL &= ~(CLK_BITS);

	u8_ID = 13;
	Timer_ErrorState = TIM_DelayUs(TIMER_0, 1000);
	if (
		(Timer_ErrorState == TIMER_E_PENDING) && (TIMER_0_CONTROL == gastr_Timer_Config[TIMER_0].u8_prescale)
		)
	{
		printf("Test Case ID: %u - PASSED \n", u8_ID);
		u8_PassedCounter++;
	...
	if (
		(Timer_ErrorState == TIMER_E_SUCCESS)
		)
	{
		printf("Test Case ID: %u - PASSED \n", u8_ID);
		u8_PassedCounter++;
