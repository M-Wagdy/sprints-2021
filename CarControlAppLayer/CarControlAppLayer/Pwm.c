...
uint8_t PwmInit(void)
{
	...
	#if (OC1A || OC1B)
		TWO_BITS_WRITE(TCCR1A,WGM10,TWO_BITS_GET(OC1_MODE,BIT0));//checked
		TWO_BITS_WRITE(TCCR1B,WGM12,TWO_BITS_GET(OC1_MODE,BIT2));//checked
	#endif

	#if OC1A
		TWO_BITS_WRITE(TCCR1A,COM1A0,OC1A_WAVEFORM_MODE);//checked
	#endif

	#if OC1B
		TWO_BITS_WRITE(TCCR1A,COM1B0,OC1B_WAVEFORM_MODE);//checked
	#endif
...
uint8_t PwmStart(uint8_t PwmChannelNumber)
{
	switch(PwmChannelNumber)
	{
		...
		case PWM_CHANNEL_OC1A:
			#if OC1A
				THREE_BITS_WRITE(TCCR1B,CS10,OC1_FREQUANCY);//checked
			#endif
			break;
